...
module.exports = {
    ...
    .addStringOption(options => options 
        .setName("title")
        .setDescription("Specify the embed's title.")
        .setRequired(false)
    )
    .addStringOption(options => options 
        .setName("image")
        .setDescription("Specify the embed's image.")
        .setRequired(false)
    )
    .addStringOption(options => options
        .setName("footertext")
        .setDescription("Specify a footer name")
        .setRequired(false)
    )
    .addStringOption(options => options
        .setName("footerimage")
        .setDescription("Specify an image link for the footer.")
        .setRequired(false)
    )
    .addBooleanOption(options => options
        .setName("disablecolor")
        .setDescription("Disable the embed's color (True)")
        .setRequired(false)
    )
    .addStringOption(options => options
        .setName("embedcolor")
        .setDescription("Specify a color in HEX format (#FFFFFF).")
        .setRequired(false)
    ),
    /**
     * 
    ...
    async execute(interaction, client) {
        ...
        //Options
        const rawmessage = options.getString("text") || null;
        ...
        if (interactionAuthor.permissionsIn(channel).has('SendMessages') && interaction.guild.members.me.permissionsIn(channel).has('SendMessages') && isfooterinvalid == false && isimageinvalid == false) {
            channel.send({embeds: [messageEmbed]})
            interaction.reply({embeds: [successEmbed], ephemeral: true})
        }  
        if(!interactionAuthor.permissionsIn(channel).has('SendMessages')) {
